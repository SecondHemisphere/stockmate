<x-layouts.app>
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-800 mb-2">Reporte de Proveedores Activos</h2>
        <p class="text-sm text-gray-600">Listado actualizado de todos los proveedores activos en el sistema.</p>
    </div>

    <div class="flex justify-between items-center mb-4">
        <!-- Botón PDF -->
        <a href="{{ route('reportes.proveedores-activos.pdf') }}"
            class="btn bg-red-600 hover:bg-red-500 text-white px-5 py-2 rounded-lg flex items-center gap-2">
            <i class="fas fa-file-pdf"></i> Descargar PDF
        </a>

        <!-- Botón Excel -->
        <a href="{{ route('reportes.proveedores-activos.excel') }}"
            class="btn bg-green-600 hover:bg-green-500 text-white px-5 py-2 rounded-lg flex items-center gap-2">
            <i class="fas fa-file-excel"></i> Descargar Excel
        </a>
    </div>

    <x-tabla-reporte :columnas="[
        ['campo' => 'id', 'titulo' => 'ID'],
        ['campo' => 'nombre', 'titulo' => 'Nombre'],
        ['campo' => 'correo', 'titulo' => 'Correo'],
        ['campo' => 'telefono', 'titulo' => 'Teléfono'],
        ['campo' => 'direccion', 'titulo' => 'Dirección'],
        ['campo' => 'estado', 'titulo' => 'Estado'],
    ]" :filas="$proveedores" />
</x-layouts.app>
